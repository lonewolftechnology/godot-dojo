...
#ifndef TOKEN_TRANSFER_H
#define TOKEN_TRANSFER_H
...
class TokenTransferSubscription : public DojoSubscription
{
    GDCLASS(TokenTransferSubscription, DojoSubscription);
    ...
    mutable std::vector<DOJO::FieldElement> native_account_addresses;
...
public:
    ...
    void set_account_addresses(const TypedArray<String>& p_addresses) {
        account_addresses = p_addresses;
        native_account_addresses.clear();
        native_account_addresses.reserve(p_addresses.size());
        for (int i = 0; i < p_addresses.size(); ++i) {
            String address_str = p_addresses[i];
            native_account_addresses.push_back(FieldElement::from_string(address_str, 32));
        }
    }
    ...
    DOJO::CArrayFieldElement get_native_account_addresses() const {
        return {native_account_addresses.data(), native_account_addresses.size()};
    }
...
protected:
    static void _bind_methods() {
        ClassDB::bind_method(D_METHOD("get_contract_addresses"), &TokenTransferSubscription::get_contract_addresses);
        ClassDB::bind_method(D_METHOD("set_contract_addresses", "p_addresses"), &TokenTransferSubscription::set_contract_addresses);
        ADD_PROPERTY(PropertyInfo(Variant::PACKED_STRING_ARRAY, "contract_addresses"), "set_contract_addresses", "get_contract_addresses");

        ClassDB::bind_method(D_METHOD("get_account_addresses"), &TokenTransferSubscription::get_account_addresses);
        ClassDB::bind_method(D_METHOD("set_account_addresses", "p_addresses"), &TokenTransferSubscription::set_account_addresses);
        ADD_PROPERTY(PropertyInfo(Variant::PACKED_STRING_ARRAY, "account_addresses"), "set_account_addresses", "get_account_addresses");

        ClassDB::bind_method(D_METHOD("get_token_ids"), &TokenTransferSubscription::get_token_ids);
        ClassDB::bind_method(D_METHOD("set_token_ids", "p_ids"), &TokenTransferSubscription::set_token_ids);
        ADD_PROPERTY(PropertyInfo(Variant::PACKED_STRING_ARRAY, "token_ids"), "set_token_ids", "get_token_ids");
    }
};

#endif //TOKEN_TRANSFER_H
