...
#ifndef TOKEN_BALANCE_H
#define TOKEN_BALANCE_H
...
class TokenBalanceSubscription : public DojoSubscription
{
    GDCLASS(TokenBalanceSubscription, DojoSubscription);
...
public:
    ...
    void set_account_addresses(const TypedArray<String>& p_addresses) {
        ...
        for (int i = 0; i < p_addresses.size(); ++i) {
            String address_str = p_addresses[i];
            native_account_addresses.push_back(FieldElement(address_str, 32).get_felt_no_ptr());
        }
    }
...
protected:
    static void _bind_methods() {
        ClassDB::bind_method(D_METHOD("get_contract_addresses"), &TokenBalanceSubscription::get_contract_addresses);
        ClassDB::bind_method(D_METHOD("set_contract_addresses", "p_addresses"), &TokenBalanceSubscription::set_contract_addresses);
        ADD_PROPERTY(PropertyInfo(Variant::PACKED_STRING_ARRAY, "contract_addresses"), "set_contract_addresses", "get_contract_addresses");

        ClassDB::bind_method(D_METHOD("get_account_addresses"), &TokenBalanceSubscription::get_account_addresses);
        ClassDB::bind_method(D_METHOD("set_account_addresses", "p_addresses"), &TokenBalanceSubscription::set_account_addresses);
        ADD_PROPERTY(PropertyInfo(Variant::PACKED_STRING_ARRAY, "account_addresses"), "set_account_addresses", "get_account_addresses");

        ClassDB::bind_method(D_METHOD("get_token_ids"), &TokenBalanceSubscription::get_token_ids);
        ClassDB::bind_method(D_METHOD("set_token_ids", "p_ids"), &TokenBalanceSubscription::set_token_ids);
        ADD_PROPERTY(PropertyInfo(Variant::PACKED_STRING_ARRAY, "token_ids"), "set_token_ids", "get_token_ids");
    }
};

#endif //TOKEN_BALANCE_H
