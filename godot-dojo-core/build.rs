...
fn handle_uniffi_dependency(dep_name: &str, udl_relative_path: &str, output_sub_dir: &str) {
    ...
    if !output.status.success() {
        ...
        panic!("uniffi-bindgen-cpp for '{}' failed:\n{}", dep_name, stderr);
    }
...
fn main(){
    // Tell Cargo to re-run if the build script changes.
    // TRICK: To always force a re-run during debugging,
    // run `FORCE_REBUILD=$(date +%s) cargo build -v` in the terminal.
    println!("cargo:rerun-if-changed=build.rs");

    let manifest_dir = env::var("CARGO_MANIFEST_DIR").unwrap();
    println!("  -> 🚀 Source directory of 'godot-dojo-core': {}", manifest_dir);
    ...
    println!("  -> 📦 --- Processing dependency: dojo-c ---");
    ...
    println!("  -> 📜 Source header: {}", source_header_path.display());
    ...
    println!("  -> 📥 Destination header: {}", dest_header_path.display());
    ...
    println!("  -> ✅ --- Copied dojo.h successfully ---\n");
}